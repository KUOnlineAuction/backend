...
module.exports.gernerateBadge = catchAsync(async () => {
  ...
  const star = { _id: "63495544a102ac2aace7158c" };
  ...
  // assignBadge
  for (let i = 0; i < user.length; i++) {
    const checkFraud = auctionController.fraudCalculator(
      user[i].totalAuctioned,
      user[i].successAuctioned,
      user[i].rating
    );
    if (checkFruad) {
