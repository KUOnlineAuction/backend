...
const savePicture = async(base64Image, folder, filename, width=1000, height=1000, quality=80) => {
    const img = base64Image.replace(/.*,/, "");
    const imageBuffer = Buffer.from(img, 'base64')
    const filePath = path.join(__dirname, '..', 'picture', folder, filename)
    await sharp(imageBuffer).resize(width,height).toFormat('jpeg').jpeg({quality: quality}).toFile(filePath)
}
