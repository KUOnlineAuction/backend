...
// cron.schedule('*/5 * * * * *', async () => {
    ...
    const billingsFound = await BillingInfo.aggregate([
        ...
        {
            $set: {auctioneerID: { $arrayElemAt: ["$involvedAuction.auctioneerID", 0]}}
        },
        {
            $project: {
                auctioneerID: 1,
    ...
    for(let el of billingsFound){
        let report = {
            ...
            description: "[Autogenerated Report] This user has past the deadline of sending the delivery info."
        }
        const newReport = await Report.create(report)
    }
    const billingsUpdate = await BillingInfo.updateMany(filter, update)
    console.log(`Found ${billingsUpdate.n} auctions past deliver deadline, Generated ${billingsUpdate.n} reports`)
})
...
cron.schedule('0 0 * * *', async () => {
    ...
    const billingsUpdate = await BillingInfo.updateMany(filter, update)
    console.log(`Found ${billingsUpdate.n} auctions past item recieved confirmation deadline, Auto completed ${billingsUpdate.n} auctions`)
})